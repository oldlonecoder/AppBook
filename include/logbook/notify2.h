...
namespace book
{
	...
	template<typename ... A>
    struct signal<void(A...)> : inner::sig_base
	{
		...
		connection_raw connect(C* object) const
		{
            ...
            inner::conn_base* conn = new inner::conn_base(this, idx);
		...
		connection_raw connect(F&& functor) const
		{
			...
			else if constexpr (sizeof(std::remove_pointer_t<f_type>) <= sizeof(void*))
			{
                ...
                using conn_t = std::conditional_t<std::is_trivially_destructible_v<F>, inner::conn_base, inner::conn_nontrivial<F>>;
                inner::conn_base* conn = new conn_t(this, idx);
				conns.emplace_back(conn);
				return { conn };
			}
			else
			{
                ...
                inner::conn_base* conn = new inner::conn_nontrivial<unique>(this, idx);
				conns.emplace_back(conn);
				return { conn };
			}
		}
